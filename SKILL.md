...
Read papers in relevance order. For high/medium-relevance papers: read full extracted text + review key tables/images. For low relevance: read abstract + methodology + conclusions only.

Generate one `.md` file per paper using the 7-section template (high/medium) or condensed 4-section template (low). **Save each file into the relevance-based subdirectory** (`高相关/`, `中相关/`, `低相关/`) under the output path.

Default output: `D:\Obsidian\file_storage\Abaqus\文献阅读整理\神经算子\<相关度>\`

### Phase 4: Master Index & Cross-Paper Synthesis

Generate two final files at the **root** of the output directory:
- **`00_总览索引.md`** — Summary table of all papers, sorted by relevance, with one-line takeaways. Use relative wikilinks: `[[高相关/01_xxx|Display Name]]`
